...
test('merchant switches one synchronized 7/30-day platform dashboard across desktop and mobile', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('[data-funnel-stage]')).toHaveCount(4)
  ...
  await expect(page.getByText('非确认发布')).toBeVisible()
  ...
  await expect(page.getByText('贴卡访问为全店入口，后续指标按美团筛选。', { exact: false })).toBeVisible()
